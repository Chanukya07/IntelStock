...
with st.sidebar:
    ...
    st.markdown("<div style='font-size:0.65rem;color:#334155;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:8px;'>NAVIGATION</div>", unsafe_allow_html=True)
...
<div class="ticker-wrap">
  <div class="ticker-content">
    ...
    <span class="up">▲ MARUTI 12,440 (+0.5%)</span> &nbsp;·&nbsp;
...
with c1:
    st.metric("NIFTY 50", "24,762", "+1.24% today", delta_color="normal")
with c2:
    st.metric("SENSEX", "81,467", "+0.87%", delta_color="normal")
with c3:
    st.metric("Portfolio P&L", "+₹1.24L", "+8.3% overall", delta_color="normal")
with c4:
    st.metric("AI Sentiment", "Bullish 🟢", "73% positive signals", delta_color="off")
...
with col_movers:
    ...
    for m in movers:
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:10px;border-radius:8px;margin-bottom:4px;border:1px solid rgba(255,255,255,0.05);">
          ...
          <div style="flex:1;">
            <div style="font-size:0.82rem;font-weight:600;color:#e2e8f0;">{m['sym']}</div>
            <div style="font-size:0.7rem;color:#64748b;">{m['sector']}</div>
          </div>
          <div style="text-align:right;">
            <div style="font-size:0.82rem;font-weight:600;color:#e2e8f0;font-family:'JetBrains Mono',monospace;">{m['price']}</div>
            <div style="font-size:0.72rem;font-weight:600;color:{color};">{arrow} {m['chg']}</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
